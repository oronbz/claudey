...
final class GhosttyHost {
    ...
    private let queue = DispatchQueue(label: "com.oronbz.Claudey.ghostty")
    ...
    private static func bringForward(_ app: NSRunningApplication) {
        ...
        NSWorkspace.shared.openApplication(at: url, configuration: configuration) { _, error in
            if let error { NSLog("Claudey could not bring Ghostty forward: \(error)") }
        }
    }
    ...
    private nonisolated func focusTerminal(hosting clients: [HerdrClientProcesses.Client]) -> Bool {
        ...
        case OSStatus(errAEEventNotPermitted):
            ...
            NSLog("Claudey has no Automation permission for Ghostty; activating the app instead")
            return false
        case let status:
            NSLog("Claudey could not check Automation permission for Ghostty: \(status)")
            return false
        }
        ...
        guard let match = terminals.first(where: { pids.contains($0.pid) || ttys.contains($0.tty) }) else {
            NSLog("Claudey found no Ghostty terminal hosting a Herdr client among \(terminals.count)")
            return false
        }
        let focused = GhosttyScripting.focus(terminalID: match.id)
        #if DEBUG
        NSLog("Claudey Ghostty terminal \(match.id) (pid \(match.pid), \(match.tty)) focused: \(focused)")
        #endif
        return focused
    }
}
...
nonisolated enum GhosttyScripting {
    ...
    private static func run(_ source: String) -> NSAppleEventDescriptor? {
        ...
        if let error {
            NSLog("Claudey could not talk to Ghostty: \(error)")
            return nil
        }
        return result
    }
